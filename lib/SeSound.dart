...
class SeSound {
  ...
  SeSound() {
    ...
    () async {
      // 読み込んだ効果音をバッファに保持
      var button1Id = await rootBundle
          .load("assets/se/Alarm.mp3")
          .then((value) => _soundPool.load(value));
      var button2Id = await rootBundle
          .load("assets/se/button2.mp3")
          .then((value) => _soundPool.load(value));
      var button3Id = await rootBundle
          .load("assets/se/button3.mp3")
          .then((value) => _soundPool.load(value));
      var button4Id = await rootBundle
          .load("assets/se/button4.mp3")
          .then((value) => _soundPool.load(value));
      var button5Id = await rootBundle
          .load("assets/se/button5.mp3")
          .then((value) => _soundPool.load(value));
      // バッファに保持した効果音のIDを以下のコンテナに入れておく
      _seContainer[SeSoundIds.button1] = button1Id;
      _seContainer[SeSoundIds.button2] = button2Id;
      _seContainer[SeSoundIds.button3] = button3Id;
      _seContainer[SeSoundIds.button4] = button4Id;
      _seContainer[SeSoundIds.button5] = button5Id;
      // 効果音を鳴らしたときに保持するためのstreamIdのコンテナを初期化
      // 対象の効果音を強制的に停止する際に使用する
      _streamContainer[button1Id] = 0;
      _streamContainer[button2Id] = 0;
      _streamContainer[button3Id] = 0;
      _streamContainer[button4Id] = 0;
      _streamContainer[button5Id] = 0;
    }();
  }
